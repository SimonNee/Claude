...
// ---------------------------------------------------------------------------
// Iteration 4 — ASM variant benchmarks (C++ baseline; bodies TBD by agentASM)
// ---------------------------------------------------------------------------

static void benchGetSpread_asm(int n) {
    OrderBook book;
    seedBook(book);
    ...
    uint64_t t0 = rdtscp();
    for (int i = 0; i < n; ++i) book.getSpread();
    uint64_t t1 = rdtscp();

    uint64_t t2 = rdtscp();
    for (int i = 0; i < n; ++i) book.getSpread_asm();
    uint64_t t3 = rdtscp();

    printResult("getSpread (C++)", n, t1 - t0);
    printResult("getSpread_asm   (C++ baseline)", n, t3 - t2);
}

static void benchCross1Level_asm(int n) {
    // C++ original
    {
        OrderBook book;
        for (int i = 0; i < n / 10; ++i) {
        ...
        uint64_t t1 = rdtscp();
        printResult("addOrder cross-1L (C++)", n, t1 - t0);
    }
    // ASM variant
    {
        OrderBook book;
        ...
        uint64_t t1 = rdtscp();
        printResult("addOrder_asm cross-1L (C++ baseline)", n, t1 - t0);
    }
}

static void benchCross5Levels_asm(int n) {
    // C++ original
    {
        OrderBook book;
        for (int i = 0; i < n / 10; ++i) {
        ...
        uint64_t t1 = rdtscp();
        printResult("addOrder cross-5L (C++)", n, t1 - t0);
    }
    // ASM variant
    {
        OrderBook book;
        for (int i = 0; i < n / 10; ++i) {
            for (int l = 1; l <= 5; ++l) book.addOrder_asm(Side::Sell, 100.0 + l * 0.05, 1.0);
        ...
        uint64_t t1 = rdtscp();
        printResult("addOrder_asm cross-5L (C++ baseline)", n, t1 - t0);
    }
}
...
int main() {
    ...
    std::cout << "\n=== Iteration 4 — ASM Baselines (C++ vs C++ _asm) ===\n\n";
